...
Run entries older than the most recent few, moved out of `.claude/log.md` so
that file stays the length it is actually read at (CLAUDE.md §5 reads the last
five). Nothing here is edited; entries are moved verbatim, oldest first.

This is the fleet's operational history: every deploy, every incident, and
every wrong inference that got corrected. It is evidence for the production
claims in the docs, so it is kept rather than pruned.
